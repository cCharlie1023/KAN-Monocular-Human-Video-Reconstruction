...
class PoseKAN(torch.nn.Module): # 封装了一个KAN神经网络模型，可以用于对数据进行拟合和预测。
    def __init__(
        self,
        layers_hidden,
        grid_size=5,
    ...
    def forward(self, x: torch.Tensor, update_grid=False): # 调用每个KANLinear层的forward方法，对输入数据进行前向传播计算输出。
        ...
        for layer in self.layers:
            ...
            x = layer(x)
        rvec = x.view(-1,3)
        Rs = self.rodriguez(rvec).view(-1, 23, 3, 3)
        return {
            "Rs": Rs
        }
...
class RodriguesModule(torch.nn.Module):
    def forward(self, rvec):
        r''' Apply Rodriguez formula on a batch of rotation vectors.
            实现了Rodrigues旋转公式,将旋转向量转换为旋转矩阵 从旋转向量（3D向量）计算旋转矩阵（3x3矩阵）的方法
            Args:
                rvec: Tensor (B, 3)            
            Returns
                rmtx: Tensor (B, 3, 3)
                旋转向量rvec的每个元素代表绕x、y、z轴的旋转分量。
                通过计算旋转角度theta(旋转向量的模),以及归一化后的旋转向量rvec,使用三角函数计算出旋转矩阵的每个元素。
                最终输出的是一个形状为(-1, total_bones, 3, 3)的张量,其中每个3x3的子矩阵代表一个骨骼的旋转矩阵。
        '''
