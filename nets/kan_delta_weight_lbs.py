...
class WeightKAN(torch.nn.Module): # 封装了一个KAN神经网络模型，可以用于对数据进行拟合和预测。
    def __init__(
        self,
        layers_hidden,
        grid_size=5,
        ...
        """
        super(WeightKAN, self).__init__()
        self.grid_size = grid_size
        self.spline_order = spline_order
        self.actvn = torch.nn.SiLU()
        ...
        for in_features, out_features in zip(layers_hidden, layers_hidden[1:]):
            self.layers.append(
                KANLinear(
                    in_features,
                    out_features=128 if out_features == 191 else out_features,
                    grid_size=grid_size,
        ...
        self.bw_fc = torch.nn.Conv1d(128, 24, 1)
    ...
    def forward(self, x: torch.Tensor, update_grid=False): # 调用每个KANLinear层的forward方法，对输入数据进行前向传播计算输出。
        ...
        返回:
            torch.Tensor: 输出张量，形状为 (batch_size, out_features)。
        """
        features = xyz_embedder(x)
        features = features.squeeze(0)

        net = features
        for i, l in enumerate(self.layers):
            net = self.actvn(self.layers[i](net))
            if i in self.skips:
                net = torch.cat((features, net), dim=1)
        net = net.unsqueeze(0).permute(0, 2, 1)
        bw = self.bw_fc(net)  
        return bw     
