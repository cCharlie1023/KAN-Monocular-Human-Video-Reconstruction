...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for id in range(len(views)):
        ...
        render_file_path = os.path.join(render_path, '{0:05d}'.format(id) + ".png")
        # print(f"Saving rendered image to: {render_file_path}")
        torchvision.utils.save_image(rendering, render_file_path)

        gt_file_path = os.path.join(gts_path, '{0:05d}'.format(id) + ".png")
        # print(f"Saving ground truth image to: {gt_file_path}")
        torchvision.utils.save_image(gt, gt_file_path)
