...
from nets.kan_delta_body_pose import PoseKAN
from nets.kan_delta_weight_lbs import WeightKAN
class  GaussianModel:
# 定义构建3D高斯协方差矩阵的函数
    def setup_functions(self):
        def build_covariance_from_scaling_rotation(scaling, scaling_modifier, rotation, transform):
            """
    build_scaling_rotation 构建3D高斯模型的尺度-旋转矩阵。
    :param s: 尺度参数。
    :param r: 旋转参数。
    :return: 尺度-旋转矩阵。
    """
    ...
    def __init__(self, sh_degree : int, smpl_type : str, motion_offset_flag : bool, actor_gender: str):
        ...
        self.max_sh_degree = sh_degree  #球谐函数的最大阶数
        self._xyz = torch.empty(0) #均值（位置）
        self._features_dc = torch.empty(0) #第一个球谐系数
        self._features_rest = torch.empty(0) #其余球谐系数
        self._scaling = torch.empty(0) #缩放向量s
        self._rotation = torch.empty(0) #旋转四元数q
        self._opacity = torch.empty(0) #不透明度
        self.max_radii2D = torch.empty(0) #投影到2D时, 每个2D gaussian最大的半径
        self.xyz_gradient_accum = torch.empty(0) #均值的累积梯度
        self.denom = torch.empty(0)
        self.optimizer = None #优化器
        self.percent_dense = 0
        self.spatial_lr_scale = 0
        self.setup_functions() #设置各种激活函数
        """
        ...
        if smpl_type == 'smpl':
            ...
            neutral_smpl_path = os.path.join('assets', 'male.pkl')
        ...
        if self.motion_offset_flag:
            # load pose correction module. shape属性用于获取数组或张量的维度信息,返回一个元组，表示数组或张量在每个维度上的大小
            total_bones = self.SMPL_NEUTRAL['weights'].shape[-1]
            self.pose_decoder = PoseKAN([69,256,256,256,69])
            self.pose_decoder.to(self.device)

            # load lbs weight module
            self.lweight_offset_decoder = WeightKAN([63,128,128,191,128])
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, atom_init_quantile : float):
        """
    从点云数据初始化模型参数。
    :param pcd: 点云数据,包含点的位置和颜色。这里是来自SMPL
    ...
    """
        self.percent_dense = training_args.percent_dense  # 设置在训练过程中，用于密集化处理的3D高斯点的比例
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, kl_threshold=0.4, t_vertices=None, iter=None):
        ...
        # 如果某些3Dgaussian的均值的梯度过大且尺度小于一定阈值，说明是欠重建，则对它们进行克隆
        # self.densify_and_clone(grads, max_grad, extent) 
        # 如果某些3Dgaussian的均值的梯度过大且尺度超过一定阈值，说明是过重建，则对它们进行切分
    ...
    #     rotation_1 = build_rotation(rotation_1_q)
    #     scaling_1 = build_scaling(scaling_1_diag)
    #     L_1 = rotation_1 @ scaling_1
    #     cov_1 = L_1 @ L_1.transpose(1, 2)

    #     # Calculate the square root of the covariance matrices
    #     sqrt_cov_0 = torch.linalg.cholesky(cov_0)
    #     sqrt_cov_1 = torch.linalg.cholesky(cov_1)

    # # Calculate the mean difference
    #     mu_diff = mu_1 - mu_0

    # # Calculate the Wasserstein distance
    #     wasserstein_dist = torch.sqrt(torch.sum((mu_diff[:, None] @ sqrt_cov_1 - mu_diff[:, None] @ sqrt_cov_0) ** 2, dim=1))
    ...
    def coarse_deform_c2source(self, query_pts, params, t_params, t_vertices, lbs_weights=None, correct_Rs=None, return_transl=False):
        ...
        joints_num = self.SMPL_NEUTRAL['weights'].shape[-1] # 24
        vertices_num = t_vertices.shape[1]  # 6890
        ...
        if lbs_weights is None:
            ...
            # 确保每个关节的权重和为 1。因为在皮肤加权中，权重通常需要归一化
            bweights = F.softmax(bweights, dim=-1)

        ### From Big To T Pose
        big_pose_params = t_params
        # 从SMPL 模型中提取变换参数，包括平移（A）、旋转（R）、平移向量（Th）和关节位置（joints）。
        # 这些参数是根据输入的姿态参数计算得出的
        A, R, Th, joints = get_transform_params_torch(self.SMPL_NEUTRAL, big_pose_params)
        # 将 A 重塑为形状 [bs, joints_num, -1]，然后与 bweights 进行矩阵乘法
        # 这个操作将根据每个关节的权重计算出最终的变换矩阵 A，这样每个顶点的变换都会受到关节权重的影响
...
def SMPL_to_tensor(params, device):
    """
    v_template 顶点模板数组，表示未变形的基础顶点位置
    shapedirs 形状方向数组, 用于描述形状变化对顶点位置的影响。chumpy是一个用于自动微分和优化的Python库
    J_regressor 另一个稀疏矩阵，用于从形状参数计算关节位置的回归器。
    kintree_table 表示关节层次结构的数组，用于描述骨骼的树状结构。
    f 通常表示面的数组, 用于描述3D网格模型的拓扑结构
    weights 每个顶点的骨骼权重数组，表示各顶点受不同骨骼的影响程度
    posedirs 姿态方向数组，用于描述姿态变化对顶点位置的影响。
    """
