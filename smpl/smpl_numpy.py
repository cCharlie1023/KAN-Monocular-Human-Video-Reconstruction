...
class SMPL():
    def __init__(self, sex, model_dir):
        ...
        model_paths = {
            ...
            # 暂改
            # 'neutral': os.path.join('assets/models/smplx/SMPLX_NEUTRAL.pkl')
            'neutral': os.path.join('assets/SMPL_NEUTRAL.pkl')
        }

        with open(model_paths[sex], 'rb') as f:
            smpl_model = pickle.load(f, encoding='latin1')
        self.J_regressor = np.array(smpl_model['J_regressor'].todense()) # (24, 6890) 稀疏矩阵，用于从形状参数计算关节位置的回归器
        self.weights = smpl_model['weights'] # (6890, 24) 每个顶点的骨骼权重数组，表示各顶点受不同骨骼的影响程度
        self.posedirs = smpl_model['posedirs'] # (6890, 3, 207) 姿态方向数组，用于描述姿态变化对顶点位置的影响
        self.v_template = smpl_model['v_template'] # (6890, 3) 顶点模板数组，表示未变形的基础顶点位置
        self.shapedirs = np.array(smpl_model['shapedirs']) # (6890, 3, 10) 形状方向数组，用于描述形状变化对顶点位置的影响
        self.faces = smpl_model['f'].astype('int32') # (13776, 3) 常表示面的数组，用于描述3D网格模型的拓扑结构
        self.kintree_table = smpl_model['kintree_table'].astype('int64') # (2, 24) 表示关节层次结构的数组，用于描述骨骼的树状结构

        id_to_col = {self.kintree_table[1, i].item(): i for i in range(self.kintree_table.shape[1])}
        self.parent = np.array([id_to_col[self.kintree_table[0, it]] for it in range(1, self.kintree_table.shape[1])])

        self.pose_shape = [24, 3] # 体整体运动位姿+关节点相对角度
        self.beta_shape = [10] # 代表高矮胖瘦、头身比等比例的10个参数
    ...
    def __call__(self, pose, beta):
        ...
        v_shaped = shapedirs.dot(beta).reshape(6890, 3) + v_template # (6890, 3) # 计算变形后的顶点
        J = self.J_regressor.dot(v_shaped)                           # (24, 3) 根据变形后的顶点计算关节位置
        ...
        # input is a rotation axis-angle vector: (1, 72), (72, 1) or (72, )
            ...
            R = np.array([rodrigues(pose_vectors[p_idx])[0] 
                            for p_idx in range(pose_vectors.shape[0])  # Rodrigues函数返回两个值：旋转矩阵和一个雅可比矩阵（用于优化），这里只取旋转矩阵
                          ], 
